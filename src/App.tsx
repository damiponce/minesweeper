          ...
          for (var j = -1; j <= 1; j++) {
            if (
              ...
              if (state.statuses[y + j][x + i].get() === status)
                switch ([i, j].join(' ')) {
                  case '-1 -1':
                    if (equal(x + 0, y - 1) && equal(x - 1, y + 0))
  ...
  function initState() {
    ...
    state.set({
      bombs: lodash.cloneDeep(
        lib.createGrid([settings.width.get(), settings.height.get()], false),
      ),
      neighbors: lodash.cloneDeep(
        lib.createGrid([settings.width.get(), settings.height.get()], 0),
      ),
      statuses: lodash.cloneDeep(
        lib.createGrid([settings.width.get(), settings.height.get()], 'hidden'),
      ),
      borders: lodash.cloneDeep(
        lib.createGrid([settings.width.get(), settings.height.get()], 'none'),
      ),
      generated: false,
      cleared: [{ x: -1, y: -1 }],
      hoveredCell: { x: -1, y: -1 },
    ...
    () =>
      state.statuses.subscribe(
        (s) => (
          state.statuses.get().map((row, y) =>
            row.map((status, x) => {
              if (status === 'revealed' && state.bombs[y][x].get()) {
          ...
          state.statuses
            ...
            settings.width.get() * settings.height.get() -
              settings.nBombs.get() &&
            (state.hoveredCell.set({ x: -1, y: -1 }), state.gameWon.set(true)),
          state.bombsLeft.set(
            settings.nBombs.get() -
              state.statuses
                .get()
  ...
  return (
    <div className='App' style={{ width: '100%', height: '100%' }}>
      ...
      >
        <span>
          x:{' '}
          {state.hoveredCell.get().x !== -1 ? state.hoveredCell.get().x : '-'}
        </span>
        <br />
        <span>
          y:{' '}
          {state.hoveredCell.get().y !== -1 ? state.hoveredCell.get().y : '-'}
        </span>
        <br />
        <span>
          b:{' '}
          {state.hoveredCell.get().x !== -1 && state.hoveredCell.get().y !== -1
            ? borders[state.hoveredCell.get().y][state.hoveredCell.get().x]
            : '-'}
        </span>
      </div>
      ...
      {/* 
            <div
                className='hamburger'
                onClick={() =>
                    settings.menuOpen.get() === 2
                        ? (settings.menuOpen.set(1),
                          setTimeout(() => settings.menuOpen.set(0), 150))
                        : (settings.menuOpen.set(1),
                          setTimeout(() => settings.menuOpen.set(2), 1))
                }
            >
            ...
            {settings.menuOpen.get() >= 1 && (
                <div
                    className={`hamburger-dropdown ${
                        settings.menuOpen.get() > 1 ? 'animate' : null
                    }`}
                >
                    <span
        ...
        <DropdownMenu.Portal>
          <DropdownMenu.Content className='DropdownMenuContent' sideOffset={5}>
            ...
            <DropdownMenu.CheckboxItem
              ...
              checked={settings.ezFlag.get()}
              onClick={(e) => e.stopPropagation()}
              onCheckedChange={settings.ezFlag.set}
            >
            ...
            <DropdownMenu.CheckboxItem
              className='DropdownMenuCheckboxItem'
              checked={settings.switchClicks.get()}
              onClick={(e) => e.stopPropagation()}
              onCheckedChange={settings.switchClicks.set}
            >
              <DropdownMenu.ItemIndicator className='DropdownMenuItemIndicator'>
                <CheckIcon />
              </DropdownMenu.ItemIndicator>
            ...
            <DropdownMenu.Item className='DropdownMenuItem'>
              <ResetIcon className='DropdownMenuItemIcon' />
              Restart game
            </DropdownMenu.Item>
            <DropdownMenu.Item className='DropdownMenuItem'>
              <ExitIcon className='DropdownMenuItemIcon' />
              Exit to menu
            </DropdownMenu.Item>
      ...
      {settings.playing.get() &&
        (state.gameOver.get() || state.gameWon.get()) && (
          <div className='main-menu'>
            ...
            <span
              className='start-btn'
              onClick={() => (settings.playing.set(false), initState())}
            >
              Play again
            </span>
          </div>
      ...
      <div
        className='blur'
        style={{
          backgroundColor: !settings.playing.get() ? '#275C75' : '#0000',
          backdropFilter:
            matchEnded || !settings.playing.get() ? 'blur(20px)' : 'none',
          pointerEvents:
      ...
      {/* <PrismaZoom
            ...
            > */}
      <div id='prisma-zoom' className='zoom-canvas'>
        <div
          key='separator-grid'
          id='separator-grid'
          ...
          style={{
            ...
            aspectRatio: settings.width.get() / settings.height.get(),
            maxWidth:
              (Math.min(window.innerWidth, window.innerHeight) - 24) * 5,
            maxHeight:
              (Math.min(window.innerWidth, window.innerHeight) - 24) * 5,
            width:
          ...
          {state.bombs.get().map((row, y) =>
            row?.map((bomb, x) => (
              <div
                ...
                id={`${x}-${y}`}
                className='cell'
                onMouseDown={(e) => playable && handleMouseDown(x, y, e)}
                onMouseUp={(e) => playable && handleMouseUp(x, y, e)}
                onMouseEnter={(e) =>
                  playable && state.hoveredCell.set({ x: x, y: y })
                }
                onMouseLeave={(e) =>
                  playable && state.hoveredCell.set({ x: -1, y: -1 })
                }
                // ref={componentRef}
              >
                {
                    ...
                    {
                      {
                        hidden: (
                          <g>
                            {(svgs as any)[borders[y][x]]}
                            {state.gameOver.get() && state.bombs[y][x].get() ? (
                              <circle fill='#111' cx='50' cy='50' r='22' />
                            ) : null}
                          </g>
                        ),
                        flagged: (
                        ...
                        revealed: (
                          <text
                            ...
                            fill='#ddd'
                          >
                            {state.neighbors[y][x].get() > 0 &&
                              state.neighbors[y][x].get()}
                          </text>
                        ),
                        exploded: (
                          <g fill='red'>
                            {(svgs as any)[borders[y][x]]}
                            <circle fill='#eee' cx='50' cy='50' r='22' />
                          </g>
                        ),
                      }[state.statuses[y][x].get()]
                    }
